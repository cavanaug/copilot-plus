...
tech-stack:
  added: [bash, bats, jq]
  patterns:
    - "TDD red-green cycle: failing tests → minimal implementation → passing tests"
    - "Env-var override for test isolation (COPILOT_REAL_BINARY)"
    - "General-purpose -- key dispatch: array/bool/string/number/object/null type dispatch"
...
key-decisions:
  - "COPILOT_REAL_BINARY env override added to allow test stub interception (hardcoded absolute path bypasses PATH-based stub)"
  - "exec $REAL_COPILOT uses hardcoded path by default; env override for testing"
  - "Type dispatch via jq type filter: array/boolean/string/number → flag injection; object/null → exit 1"

patterns-established:
  - "Test stub pattern: COPILOT_REAL_BINARY env var points to stub binary; stub writes args to stub_args file"
  - "bats setup/teardown: BATS_TMPDIR for isolation, stub_args cleaned in teardown"
